...
def display_confusion_matrix(cmat, score, precision, recall, titlestring, img_name):
    ...
    if len(titlestring) > 0:
        ax.text(20, -3, titlestring, fontdict={'fontsize': 30, 'horizontalalignment':'right', 'verticalalignment':'top', 'color':'#804040'})
    plt.savefig(img_name)

cm = confusion_matrix(y_true, y_pred, labels=sorted(set(y_true)))
pd.DataFrame(cm).to_csv(f"{path}/cm_{model}.csv")
img = display_confusion_matrix(cm, f1, prec, recall, "", img_name=f"{path}/cm_{model}.png")
